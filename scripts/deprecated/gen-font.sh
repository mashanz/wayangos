#!/bin/bash
# Extract 8x16 font from Linux console font (VGA ROM font)
# Generate C header with bitmap data for ASCII 32-126

cat > ~/wayangos-build/gen_font.c << 'FONTGEN'
#include <stdio.h>

/* VGA 8x16 ROM font - standard CP437 ASCII subset (32-126) */
/* Each character is 16 bytes (16 rows of 8 pixels) */
/* This is the standard VGA BIOS font used by Linux fbcon */

static const unsigned char vga8x16[][16] = {
...
int main(void) {
    printf("/* Auto-generated VGA 8x16 font - ASCII 32-126 */\n");
    printf("static const unsigned char font8x16[95][16] = {\n");
    for (int i = 0; i < 95; i++) {
        printf("  {");
        for (int j = 0; j < 16; j++) {
            printf("0x%02X", vga8x16[i][j]);
            if (j < 15) printf(",");
        }
        printf("}%s /* %c */\n", i<94?",":"", (i+32>=33&&i+32<=126)?(char)(i+32):' ');
    }
    printf("};\n");
    return 0;
}
...
cd ~/wayangos-build
gcc -o gen_font gen_font.c
./gen_font > font8x16.h
echo "Font header generated: $(wc -l font8x16.h) lines"
head -3 font8x16.h
